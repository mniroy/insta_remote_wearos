...
import com.arashivision.sdkcamera.camera.model.AccelerateFrequency
...
fun setCaptureSettingValue(
    ...
    return when (captureSetting) {
        ...
        LATENCY -> instaCameraManager.setLatency(captureMode, value as Latency, checker)
        ACCELERATE_FREQUENCY -> instaCameraManager.setAccelerateFrequency(captureMode, value as AccelerateFrequency, checker)
    }
}

fun getCaptureSettingValue(captureMode: CaptureMode, captureSetting: CaptureSetting): Any {
    Log.d("InstaCameraManagerExt", "getCaptureSettingValue   captureMode=$captureMode   captureSetting=$captureSetting")
    return when (captureSetting) {
        ...
        RAW_TYPE -> {
            val rawType = instaCameraManager.getRawType(captureMode)
            Log.d("WDebug", "InstaCameraManager getRawType = $rawType")
            rawType
        }
        ...
        PHOTO_HDR_TYPE -> {
            ...
            Log.d("InstaCameraManagerExt", "photoHdrType ==> $photoHdrType")
            photoHdrType
        }
        LIVE_BITRATE -> instaCameraManager.getLiveBitrate(captureMode)
        I_LOG -> instaCameraManager.getILogStatus(captureMode)
        LATENCY -> instaCameraManager.getLatency(captureMode)
        ACCELERATE_FREQUENCY -> instaCameraManager.getAccelerateFrequency(captureMode)
    }
}
...
fun getCaptureSettingSupportList(
    ...
    return when (captureSetting) {
        ...
        LATENCY -> instaCameraManager.getSupportLatencyList(captureMode).sortedBy { it.nativeValue }
        ACCELERATE_FREQUENCY -> instaCameraManager.getSupportAccelerateFrequencyList(captureMode).sortedBy { it.nativeValue }
    }
}