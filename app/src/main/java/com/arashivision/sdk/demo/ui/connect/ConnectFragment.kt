...
class ConnectFragment : BaseFragment<FragmentConnectBinding, ConnectViewModel>() {
    ...
    private val fL: Logger = XLog.tag("File").build()
    ...
    override fun initView() {
        ...
        val lastConnectDeviceInfo = viewModel.getLastConnectDeviceInfo()
        binding.tvLastConnectDeviceValue.text = "${lastConnectDeviceInfo.first} ${lastConnectDeviceInfo.second}"
    ...
    override fun initListener() {
        ...
        binding.tvWakeup.setOnClickListener {
            viewModel.wakeupLastConnectDevice()
        }
    }

    private fun printFile(file: File, level: Int = 0) {
        if (!file.isDirectory) return
        for (i in 0 until level) print("   ")
        println(file.name)
        context?.filesDir?.listFiles()?.forEach {
            if (it.isDirectory) {
                printFile(it, level + 1)
            } else {
    ...
    override fun onEvent(event: BaseEvent?) {
        ...
        when (event) {
            ...
            is ConnectEvent.ScanDeviceEvent -> {
                logger.d("ScanDeviceEvent status ==>" + event.status)
                when (event.status) {
                    EventStatus.START -> {
            ...
            is ConnectEvent.ConnectDeviceEvent -> {
                logger.d("ConnectDeviceEvent status ==>" + event.status)
                when (event.status) {
    ...
    private fun updateCameraInfoUi(connectType: Int) {
        ...
        binding.tvBleWakeupStateValue.text = getString(
            when (instaCameraManager.wakeupBleState) {
                InstaCameraManager.WAKEUP_BLE_STATE_ON -> R.string.text_ble_wakeup_state_on
                InstaCameraManager.WAKEUP_BLE_STATE_OFF -> R.string.text_ble_wakeup_state_off
                else -> R.string.text_ble_wakeup_state_unknown
            }
        )
