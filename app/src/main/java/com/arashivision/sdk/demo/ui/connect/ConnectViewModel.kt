...
class ConnectViewModel : BaseViewModel() {
    ...
    companion object {
        ...
        private const val SP_LAST_DEVICE_TYPE = "last_device_type"
        private const val SP_LAST_DEVICE_SERIAL = "last_device_serial"
    }
    ...
    fun getLastConnectDeviceInfo(): Pair<String, String> {
        return Pair(SPUtils.getString(SP_LAST_DEVICE_TYPE, ""), SPUtils.getString(SP_LAST_DEVICE_SERIAL, ""))
    }

    fun wakeupLastConnectDevice() {
        val lastConnectDeviceInfo = getLastConnectDeviceInfo()
        if (lastConnectDeviceInfo.first.isEmpty()) return
        instaCameraManager.stopWakeUpBle()
        instaCameraManager.startWakeUpBle(lastConnectDeviceInfo.first, lastConnectDeviceInfo.second)
    }
    ...
    private fun bindNetwork(): Boolean = kotlin.runCatching {
        ...
        logger.d("wiFiNetwork bind exception  ${it.message}")
    }.getOrDefault(false)
        ...
        when (connectType) {
            InstaCameraManager.CONNECT_TYPE_BLE -> {
                if (enabled && connectingBleDevice != null) {
                    SPUtils.putString(SP_LAST_DEVICE_TYPE, instaCameraManager.cameraType)
                    SPUtils.putString(SP_LAST_DEVICE_SERIAL, instaCameraManager.cameraSerial.substring(0.coerceAtLeast(instaCameraManager.cameraSerial.length - 6)))
                    emitEvent(ConnectEvent.ConnectDeviceEvent(EventStatus.SUCCESS, InstaCameraManager.CONNECT_TYPE_BLE))
