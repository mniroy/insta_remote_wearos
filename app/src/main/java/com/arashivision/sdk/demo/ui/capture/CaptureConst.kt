...
import com.arashivision.sdkcamera.camera.model.ResolutionType
...
fun getCaptureSettingNameResId(captureSetting: CaptureSetting): Int {
    return when (captureSetting) {
        ...
        CaptureSetting.LATENCY -> R.string.capture_setting_latency
        CaptureSetting.ACCELERATE_FREQUENCY -> R.string.capture_setting_latency
    }
}
...
fun getCaptureSettingValueName(
    ...
    return when (captureSetting) {
        ...
        CaptureSetting.RECORD_RESOLUTION -> {
            ...
            val string = "${resolution.width}_${resolution.height}_${resolution.fps}${if (resolution.type == ResolutionType.PLUS) "_PLUS" else ""}"
            string
        }
        ...
        CaptureSetting.LATENCY -> {
            val latency: Latency = value as Latency
            "${latency.nativeValue}s"
        }

        CaptureSetting.ACCELERATE_FREQUENCY -> {
            val accelerateFrequency = value as AccelerateFrequency
            if (accelerateFrequency.nativeValue == 0) context.getString(R.string.auto) else "x${accelerateFrequency.nativeValue}"
        }
    }
}
