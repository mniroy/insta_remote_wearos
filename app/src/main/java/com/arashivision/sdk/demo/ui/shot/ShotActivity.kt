...
class ShotActivity : BaseActivity<ActivityShotBinding, ShotViewModel>() {
    ...
    override fun initView() {
        super.initView()
        binding.btnWork.text = if(viewModel.isCameraWorking) getString(R.string.shot_capture_stop) else getString(R.string.shot_capture_start)
    ...
    override fun onEvent(event: BaseEvent) {
        super.onEvent(event)
        when (event) {
            is ShotEvent.InitShotEvent -> {
                when (event.status) {
                    EventStatus.START -> showLoading()
                    EventStatus.SUCCESS -> hideLoading()
                    else -> {}
                }
            }
