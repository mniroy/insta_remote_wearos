...
class WorkPlayActivity : BaseActivity<ActivityWorkPlayBinding, WorkPlayViewModel>() {
            ...
            is WorkPlayEvent.PureShotStitchEvent -> {
                when (event.status) {
                    ...
                    EventStatus.SUCCESS -> {
                        ...
                        binding.tvPureShotStitch.text =
                            getString(R.string.player_show_original_image)
                        playImage(true)
                    }
    ...
    private fun initImagePlayerView() {
        ...
        binding.imagePlayerView.setPlayerViewListener(object : PlayerViewListener {
            ...
            override fun onLoadingFinish() {
                ...
                binding.tvHdrStitch.visibility =
                    if (mWorkWrapper!!.isHDRPhoto) View.VISIBLE else View.GONE
                binding.tvPureShotStitch.visibility =
                    if (mWorkWrapper!!.supportPureShot()) View.VISIBLE else View.GONE
            }
    ...
    private fun playImage(showHdr: Boolean = false, showPureShot: Boolean = false) {
        initImagePlayerView()
        val builder: ImageParamsBuilder = viewModel.createPlayerParamsBuilder(
            mWorkWrapper!!,
            showHdr,
            showPureShot
        ) as ImageParamsBuilder
    ...
    private fun playVideo() {
        ...
        val builder: VideoParamsBuilder =
            viewModel.createPlayerParamsBuilder(mWorkWrapper!!) as VideoParamsBuilder
    ...
    override fun initListener() {
        ...
        binding.pickExportSetting.setOnApplyClickListener { data ->
            mWorkWrapper?.let {
                val width = (data[0] as Size).width
                val height = (data[0] as Size).height
                if (it.isVideo) {
                    // 导出视频对手机性能要求较高,实际项目中导出时建议关闭播放器，否则容易出现oom
                    // 注意：关闭播放器指的是调用videoPlayerView.destroy()方法，恢复播放则需要重新创建一个新的InstaVideoPlayerView对象
                    val fps = data[1] as Int
                    viewModel.exportVideo(
                        it,
                        binding.videoPlayerView,
                        if (width == -1) mWorkWrapper!!.width else width,
                        if (height == -1) mWorkWrapper!!.height else height,
                        if (fps == -1) mWorkWrapper!!.fps.toInt() else fps,
                        data[3] as Int,
                        data[2] as Boolean
                    )
                } else {
        ...
        binding.tvHdrStitch.setOnClickListener {
            mWorkWrapper?.let { work ->
                if (viewModel.hdrStitchPath.isEmpty()) {
                    ...
                    binding.tvHdrStitch.text =
                        getString(if (viewModel.showHdr) R.string.player_show_original_image else R.string.player_hdr_stitch_play)
                }
            }
        }
        ...
        binding.tvPureShotStitch.setOnClickListener {
            mWorkWrapper?.let { work ->
                if (viewModel.pureShotStitchPath.isEmpty()) {
                    ...
                    binding.tvPureShotStitch.text =
                        getString(if (viewModel.showPureShot) R.string.player_show_original_image else R.string.player_pure_shot_stitch_play)
                }
            }
        }
    }
    ...
    private fun playerSettingData(): List<PickData> {
        ...
        // 副码流播放
        val subStreamPlaybackTitle = getString(R.string.player_setting_sub_stream_playback)
        val isLrvEnable = viewModel.isLrvEnable
        val subStreamPickData = PickData(
            mWorkWrapper?.isVideo == true,
            subStreamPlaybackTitle,
            if (isLrvEnable) 0 else 1,
            switchData.map { getString(it.first) to it.second })
        {
            viewModel.isLrvEnable = it as Boolean
            if (mWorkWrapper?.isVideo == true) {
                binding.videoPlayerView.isLrvEnable = it
            } else {
                // InstaImagePlayerView不支持副码流播放
            }
        }
        dataList.add(subStreamPickData)
        ...
        val dePurpleFilterPickData = PickData(
            mWorkWrapper?.isVideo == true,
            dePurpleFilterTitle,
            if (isDePurpleFilterOn) 0 else 1,
            switchData.map { getString(it.first) to it.second }
        ) {
            viewModel.isDePurpleFilterOn = it as Boolean
            if (mWorkWrapper?.isVideo == true) {
                binding.videoPlayerView.isDePurpleFilterOn = it
            } else {
                // InstaImagePlayerView暂时不支持去紫边功能
            }
        }
        dataList.add(dePurpleFilterPickData)

        // 色彩增强
        val colorPlusTitle = getString(R.string.player_setting_color_plus)
        val isColorPlusEnable = viewModel.isColorPlusEnable
        val colorPlusPickData = PickData(
            true,
            colorPlusTitle,
            if (isColorPlusEnable) 0 else 1,
            switchData.map { getString(it.first) to it.second }
        ) {
            viewModel.isColorPlusEnable = it as Boolean
            if (mWorkWrapper?.isVideo == true) {
                binding.videoPlayerView.setColorPlusEnabled(it)
            } else {
                binding.imagePlayerView.setColorPlusEnabled(it)
            }
        }
        dataList.add(colorPlusPickData)

        // 消色差
        val imageFusionTitle = getString(R.string.player_setting_image_fusion)
        val isImageFusion = viewModel.isImageFusion
        val imageFusionPickData = PickData(
            true,
            imageFusionTitle,
            if (isImageFusion) 0 else 1,
            switchData.map { getString(it.first) to it.second }
        ) {
            viewModel.isImageFusion = it as Boolean
            if (mWorkWrapper?.isVideo == true) {
                binding.videoPlayerView.isImageFusion = it
            } else {
                binding.imagePlayerView.isImageFusion = it
            }
        }
        dataList.add(imageFusionPickData)


        // 动态拼接
        val dynamicStitchTitle = getString(R.string.player_setting_dynamic_stitch)
        val isDynamicStitch = viewModel.isDynamicStitch
        val dynamicStitchPickData = PickData(
            true,
            dynamicStitchTitle,
            if (isDynamicStitch) 0 else 1,
            switchData.map { getString(it.first) to it.second }
        ) {
            viewModel.isDynamicStitch = it as Boolean
            if (mWorkWrapper?.isVideo == true) {
                binding.videoPlayerView.isDynamicStitch = it
            } else {
                binding.imagePlayerView.isDynamicStitch = it
            }
        }
        dataList.add(dynamicStitchPickData)
        ...
        val screenRatioPosition = screenRatioOptions.indexOfFirst {
            ...
            val ratio2: Int = (1000 * (viewModel.screenRatio[0].toFloat() / viewModel.screenRatio[1])).toInt()
            ratio == ratio2
        }
        val screenRatioPickData = PickData(
            viewModel.renderModelType != PlayerParamsBuilder.RENDER_MODE_PLANE_STITCH,
            screenRatioTitle,
            screenRatioPosition,
            screenRatioOptions
        ) {
            val screenRatio = it as IntArray
        ...
        dataList.add(screenRatioPickData)
        ...
        val stabTypePickData = PickData(
            true,
            stabTypeTitle,
            stabTypePosition,
            stabTypeOptions.map { getString(it.first) to it.second }
        ) {
            val stabType = it as Int
        ...
        val renderModePickData = PickData(
            true,
            renderModeTitle,
            renderModePosition,
            renderModeOptions.map { getString(it.first) to it.second }
        ) {
            val renderModeType = it as Int
        ...
        dataList.add(renderModePickData)
        ...
        val offsetTypePickData = PickData(
            true,
            offsetTypeTitle,
            offsetTypePosition,
            offsetTypeOptions.map { getString(it.first) to it.second }
        ) {
            val offsetType: OffsetType = it as OffsetType
        ...
        dataList.add(offsetTypePickData)
    ...
    private fun exportParamsSettingData(): List<PickData> {
        ...
        val resolutionList = sizeOptionsList.filter {
            it.third[0] == screenRatio[0] && it.third[1] == screenRatio[1] && it.second.width <= mWorkWrapper!!.width && it.second.height <= mWorkWrapper!!.height || it.first == ORIGINAL_SIZE_OPTION.first
        }.map { it.first to it.second }

        val resolutionSelect = resolutionList.map { it.second }
            .indexOfFirst { it.width == mWorkWrapper!!.width && it.height == mWorkWrapper!!.height }
        ...
        if (mWorkWrapper!!.isVideo) {
            ...
            val bitrateList =
                getBitrateOptionsList(this).filter { it.second <= mWorkWrapper!!.bitrate }
