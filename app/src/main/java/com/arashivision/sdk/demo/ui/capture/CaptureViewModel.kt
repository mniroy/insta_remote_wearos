...
class CaptureViewModel : BaseViewModel(), IPreviewStatusListener, ICaptureStatusListener {
    ...
    fun getCaptureSettingSupportValueList(captureSetting: CaptureSetting): List<Any> {
        ...
        return when (captureSetting) {
            ...
            CaptureSetting.LATENCY -> {
                val sortedBy = instaCameraManager.getSupportLatencyList(captureMode).sortedBy { it.nativeValue }
                sortedBy
            }

            CaptureSetting.ACCELERATE_FREQUENCY -> {
                val sortedBy = instaCameraManager.getSupportAccelerateFrequencyList(captureMode).sortedBy { it.nativeValue }
                sortedBy
            }
        }
    }
    ...
    fun getCaptureParams(): CaptureParamsBuilderV2 {
        return CaptureParamsBuilderV2().apply {
            if (Pref.getCaptureActivityPlaneStitch()) {
                this.renderModel = RenderModel.PLANE_STITCH
                this.setScreenRatio(2, 1)
            }
    ...
    fun initCapture() {
        ...
        viewModelScope.launch {
            ...
            instaCameraManager.setPreviewStatusChangedListener(this@CaptureViewModel)
            instaCameraManager.setCaptureStatusListener(this@CaptureViewModel)
            ...
            if (instaCameraManager.supportConfig?.supportNewCaptureControlFlow() == false) {
            ...
            emitEvent(
                CaptureEvent.InitCaptureEvent(
                    ...
                    isSingleClickAction = isSingleClickAction()
                )
            )
        }
    }

    fun updateSDRemaining(fetchOptions: Boolean = false) {
        viewModelScope.launch {
            if (fetchOptions) {
                // 延迟一秒,等待生效
                delay(2000)
                instaCameraManager.fetchCameraOptions(object : ICameraOperateCallback {
                    override fun onSuccessful() {
                        val remaining = instaCameraManager.getRemaining(cameraOfflineData.currentCaptureMode)
                        emitEvent(CaptureEvent.UpdateRemainingEvent(cameraOfflineData.currentCaptureMode, remaining, instaCameraManager.cameraStorageFreeSpace))
                    }
    ...
    fun switchCaptureMode(position: Int) {
        ...
        viewModelScope.launch {
            ...
            if (!result) {
                emitEvent(CaptureEvent.SwitchCaptureModeEvent(EventStatus.FAILED, captureMode, isSingleClickAction(captureMode)))
                return@launch
            }
            ...
            //直播时需要把预览流的类型设置为直播类型，需要重启预览流
            if (captureMode.isLiveMode && instaCameraManager.supportConfig?.supportNewCaptureControlFlow() == false) {
                openPreviewStream(isReopen = true, isLive = false)
            }
            updateSDRemaining()
            emitEvent(CaptureEvent.SwitchCaptureModeEvent(EventStatus.SUCCESS, captureMode, isSingleClickAction(captureMode)))
        }
    }
    ...
    private fun startLive() {
        ...
        instaCameraManager.startLive(
            liveRtmp,
            NetworkManager.mobileNet?.networkHandle ?: -1,
    ...
    fun isSingleClickAction(captureMode: CaptureMode = cameraOfflineData.currentCaptureMode): Boolean {
        return captureMode.isPhotoMode && captureMode !in listOf(
    ...
    fun startCapture() {
        ...
        suspend fun reopen(isLive: Boolean, isRecord: Boolean, onFinish: (() -> Unit)? = null) {
            if (instaCameraManager.supportConfig?.supportNewCaptureControlFlow() == false) {
                openPreviewStream(true, isLive, isRecord, onFinish)
            } else {
                onFinish?.invoke()
            }
        }
        viewModelScope.launch {
            ...
            if (isSingleClickAction()) {
                reopen(isLive = false, isRecord = false)
                takePhotos(currentCaptureMode)
            } else {
                // 处理录制/直播逻辑
                when {
                    currentCaptureMode.isLiveMode && !isLiving -> {
                        reopen(isLive = true, isRecord = false){
                            startLive()
                        }
                    }
                    ...
                    !currentCaptureMode.isLiveMode && !instaCameraManager.isCameraWorking -> {
                        reopen(isLive = false, isRecord = true)
                        startRecord(currentCaptureMode)
                    }

                    !currentCaptureMode.isLiveMode && instaCameraManager.isCameraWorking -> stopRecord(currentCaptureMode)
    ...
    private fun startRecord(captureMode: CaptureMode) {
        ...
        if (!InstaCameraManager.getInstance().isSdCardEnabled) {
            emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = captureMode, isSingleClickAction = isSingleClickAction(), CaptureEvent.CaptureStatus.SD_DISABLE))
            return
        }
        when (captureMode) {
    ...
    private suspend fun setOfflineCaptureSettingValueToCamera(): Boolean {
        ...
        return suspendCancellableCoroutine {
            ...
            with(instaCameraManager) {
                ...
                if (instaCameraManager.supportConfig?.supportNewCaptureControlFlow() == true) {
    ...
    private suspend fun openPreviewStream(isReopen: Boolean = false, isLive: Boolean = false, isRecord: Boolean = false, onFinish: (() -> Unit)? = null): Boolean {
        val suspendCancellableCoroutine = suspendCancellableCoroutine {
            ...
            openPreviewStreamListener = { success ->
                ...
                if (success) emitEvent(CaptureEvent.RestartPlayerViewEvent(onFinish))
                it.resume(success)
                openPreviewStreamListener = null
            }
            val previewStreamType =
                if (isLive) InstaCameraManager.PREVIEW_TYPE_LIVE else if (isRecord) InstaCameraManager.PREVIEW_TYPE_RECORD else InstaCameraManager.PREVIEW_TYPE_NORMAL
            if (isReopen) {
                emitEvent(CaptureEvent.ReopenPreviewStreamEvent)
                instaCameraManager.restartPreviewStream(previewStreamType)
//                instaCameraManager.closePreviewStream()
//                instaCameraManager.startPreviewStream(previewStreamType)
            } else {
                instaCameraManager.startPreviewStream(previewStreamType)
            }
        }
    ...
    private suspend fun checkCameraSensorMode(): Boolean {
        ...
        if (instaCameraManager.isCameraDualSensorMode) {
    ...
    override fun onIdle() {
        ...
        emitEvent(CaptureEvent.CameraPreviewStreamClosed)
    }
    ...
    override fun onCaptureStarting() {
        emitEvent(
            CaptureEvent.CameraCaptureEvent(
                captureMode = cameraOfflineData.currentCaptureMode,
                isSingleClickAction = isSingleClickAction(),
    ...
    override fun onCaptureStopping() {
        emitEvent(
            CaptureEvent.CameraCaptureEvent(
                captureMode = cameraOfflineData.currentCaptureMode,
                isSingleClickAction = isSingleClickAction(),
                CaptureEvent.CaptureStatus.STOPPING
            )
        )
    }
    ...
    private fun onCaptureFinishEnd() {
        viewModelScope.launch {
            ...
            if (cameraOfflineData.currentCaptureMode.isVideoMode || cameraOfflineData.currentCaptureMode in arrayOf(
                    CaptureMode.INTERVAL_SHOOTING,
                    CaptureMode.STARLAPSE_SHOOTING
                )
            ) {
                openPreviewStream(isReopen = true)
            } else {
    ...
    override fun onCaptureError(i: Int) {
        emitEvent(CaptureEvent.CameraCaptureEvent(captureMode = cameraOfflineData.currentCaptureMode, isSingleClickAction = isSingleClickAction(), errorCode = i))
    }
    ...
    fun cameraPreviewStreamParamsChanged(playerView: InstaCapturePlayerView) {
        ...
        if (playerView.isPlaying) {
            ...
            val stabOffset = assetInfoStab?.let { InstaCapturePlayerView.getPlayerOffsetData(it).offsetV1 }

            // WindowCropInfo参数变化刷新Offset
            val shouldUpdateWindowCrop = if (assetInfo != null || stabOffset != null) shouldUpdateWindowCrop(playerView, assetInfo!!, stabOffset!!) else false

            val windowCropInfo = if (shouldUpdateWindowCrop && assetInfo != null) {
    ...
    fun supportPreview(): Boolean {
        val captureMode = cameraOfflineData.currentCaptureMode
        if (!instaCameraManager.isCameraWorking) return true
        return captureMode != CaptureMode.TIME_SHIFT
                && captureMode != CaptureMode.TIMELAPSE
                && instaCameraManager.supportConfig?.supportNewCaptureControlFlow() == true
    }
}
