...
class MainViewModel : BaseViewModel() {
    ...
    fun hasWindowPermission(activity: Activity): Boolean {
        return XXPermissions.isGranted(activity,Permission.SYSTEM_ALERT_WINDOW)
    }

    fun checkWindowPermission(activity: Activity, callback: (Boolean) -> Unit) {
        XXPermissions.with(activity)
            .permission(Permission.SYSTEM_ALERT_WINDOW).request { _, allGranted ->
                callback(allGranted)
            }
    }
